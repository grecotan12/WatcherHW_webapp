...
public class TheController {
    ...
    public String getHw(@RequestParam String infoType) {
       return getHardwareService.runScript("./python_scripts/hw-test.py", infoType);
    }

    @CrossOrigin(origins = "http://localhost:3000")
    @GetMapping("/getDiskSpeed")
    public String getDiskSpeed(@RequestParam String absolutePath) {
        return getHardwareService.runScript("./python_scripts/disk_speed_lib/diskspeed.py", absolutePath);
    }

    @CrossOrigin(origins = "http://localhost:3000")
    @GetMapping("/monitorCpu")
    public String monitorCpu(@RequestParam String cpuInfoType) {
        return getHardwareService.runScript("./python_scripts/run_admin.py",  cpuInfoType);
    }

    @CrossOrigin(origins = "http://localhost:3000")
    @GetMapping("/getGpuPowerLoadsTemp")
    public String getGpuPowerLoadsTemp() {
        return getHardwareService.runScript("./python_scripts/run_admin.py",  "gpu_power");
    }

    @CrossOrigin(origins = "http://localhost:3000")
    @GetMapping("/checkLibs")
    public String checkLibs() {
        return getHardwareService.runScript("./python_scripts/check_libs.py", "");
    }

    @CrossOrigin(origins = "http://localhost:3000")
    @GetMapping("/getNetworkSpeed")
    public String getNetworkSpeed() {
        return getHardwareService.runScript("./python_scripts/test_speed.py", "");
    }
}
