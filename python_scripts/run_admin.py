...
if not admin.isUserAdmin():
    if len(sys.argv) == 2:
        if sys.argv[1] == 'cpu_temps':
            ...
            admin.runAsAdmin(f"pythonw ./python_scripts/temp_app.pyw -i temp") # Run command as root
        if sys.argv[1] == 'cpu_clocks':
            print(json.dumps("Getting cpu details..."))
            admin.runAsAdmin(f"pythonw ./python_scripts/temp_app.pyw -i clock") # Run command as root
        if sys.argv[1] == 'cpu_powers':
            print(json.dumps("Getting cpu details..."))
            admin.runAsAdmin(f"pythonw ./python_scripts/temp_app.pyw -i pow") # Run command as root
        if sys.argv[1] == 'cpu_vols':
            print(json.dumps("Getting cpu details..."))
            admin.runAsAdmin(f"pythonw ./python_scripts/temp_app.pyw -i vol") # Run command as root
